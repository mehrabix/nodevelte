...
<div dir="rtl" class="flex justify-center flex-col items-center h-screen">
    ...
    {#each transactions as t}
        <div class="flex items-center justify-between gap-x-5 mt-6">
            ...
            <button class="btn btn-error" on:click={() => deleteValue(t._id)}>حذف</button
            >
        </div>
    {/each}
    {#if valueAdded && setTimeout(() => { valueAdded = false }, 3000)}
    ...
    {/if}
      {#if valueDeleted && setTimeout(() => { valueDeleted = false }, 3000)}
